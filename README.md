...
https://youtu.be/8sp9zKFUKnI

## Azure managed services

### Azure Cosmos DB (MongoDB API) 
– Fully managed NoSQL database that supports MongoDB drivers and queries.

– Good fit: automatic scaling, global distribution, built-in backups, high availability SLA, no server management.

### Azure Service Bus 
– Fully managed message broker with queues and topics, similar role to RabbitMQ.

– Good fit: durable messaging, at-least-once delivery, automatic scaling, geo-disaster recovery, dead-letter queues, and no need to manage brokers or storage.

## Modified yaml file FINAL
```
...
spec:
  ...
  replicas: 3 # 3 MongoDB replicas for high availability
  selector:
    matchLabels:
  ...
  volumeClaimTemplates:
    - metadata:
      ...
      spec:
        ...
        resources:
          requests:
            storage: 5Gi # Size of PVC per pod
---
# Headless Service to expose MongoDB
apiVersion: v1
kind: Service
metadata:
  name: mongodb
spec:
  clusterIP: None # Headless service for stable DNS (mongodb-0.mongodb, etc.)
  ports:
...
spec:
  ...
  replicas: 1 # Single replica (can be increased later if needed)
  selector:
    matchLabels:
      app: rabbitmq
  template:
        ...
        - name: rabbitmq
          ...
          resources: # Resource limits and requests
            ...
            limits:
              cpu: 250m
              memory: 256Mi
          volumeMounts: # Mount configuration for RabbitMQ plugins and data
            - name: rabbitmq-enabled-plugins
              mountPath: /etc/rabbitmq/enabled_plugins
              subPath: enabled_plugins
            - name: rabbitmq-data
              mountPath: /var/lib/rabbitmq # Persistent RabbitMQ data
      volumes:
        - name: rabbitmq-enabled-plugins
          configMap:
            name: rabbitmq-enabled-plugins
  ...
  volumeClaimTemplates:
    - metadata:
      ...
      spec:
        ...
        resources:
          requests:
            storage: 5Gi # Persistent storage per RabbitMQ pod
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: order-service
          image: kandakji/order-service:v1 # Custom image for order service
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: makeline-service
          image: kandakji/makeline-service:v1 # Custom Makeline service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: product-service
          image: kandakji/product-service:v1 # Custom product service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-front
          image: kandakji/store-front:v1 # Front-end store application
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-admin
          image: kandakji/store-admin:v1 # Admin interface for store
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: ai-service
          image: kandakji/ai-service-l8:v1
          ports:
            - containerPort: 5001
          env:
